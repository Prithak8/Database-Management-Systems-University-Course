...
DELIMITER $$   #can use any number or charecter like 1241 to encaptulate procedure dilimiter
#DROP PROCEDURE IF EXISTS address_selection; #optional line just incase the procedure is already created
CREATE PROCEDURE address_selection()
SELECT * FROM students where address = 'mumbai';
$$
/* calling a procedure*/
call address_selection();
...
use unidb;
#comparing user and local variable
DELIMITER $$
...
CREATE TABLE table1 (value VARCHAR(50) NULL DEFAULT NULL);
DELIMITER $$ 
...
BEGIN
    ...
    loop_label:  LOOP
        ...
        END  IF;
          SET  x = x + 1;
        IF  (x mod 2) THEN     #when 0 false when 1 true
            ITERATE  loop_label;
        ELSE
...
SET count = 1;
WHILE count<= 10 do
   INSERT INTO Cars VALUES(count, concat('Car-',CAST(count as char)), count*100); #cast helps to change datatype from integer to character
   SET count = count + 1;
END while;
end$$
call while_loop();
...
SET count = 0;
WHILE count< 10 do
   SELECT * FROM Cars limit 2 offset count;   #limit fixes the value of how many tuple we want to see
   SET count = count + 2;						#offset: from where we want to see
END while;
end$$
call while_loop();
