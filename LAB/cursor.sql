...
DELIMITER //   /*this is also a valid delimiter   */
...
#### EXPERIMENT #### MULTIPLE CURSOR (NOT NEXTEXD)
...
BEGIN
    ...
    CREATE TEMPORARY TABLE userNames  #TEMPORARY TABLE CREATION IE NOT BEING STORED PERMANENTLY ONLY FOR THIS QUESTION
    ...
    BEGIN
        ...
        OPEN cursor_a;
            ...
            cursor_a_loop: LOOP
                 ...
                 IF p_first_name IS NOT NULL AND p_first_name <> "" THEN   #IF p first name is not null and not equal to 'empty'
    ...
    BEGIN
        DECLARE done BOOLEAN DEFAULT false;
        DECLARE p_first_name VARCHAR(20);
        DECLARE cursor_a CURSOR FOR SELECT student_name FROM students LIMIT 4,1; #offset 4, count 1
        DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
...
END LOOP;
CLOSE emp_csr;
SET l_done=0;   #manually setting the value to zero so that we come out of the loop
